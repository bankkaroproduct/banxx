...
export function ComparePanel({ open, onOpenChange, preSelectedCard }: ComparePanelProps) {
  ...
  const renderCellValue = (card: any, row: any) => {
    ...
    if (row.type === 'tags') return <div className="flex flex-wrap gap-2">{(Array.isArray(value) ? value : []).map((t: any) => <span key={t.id || t.name} className="px-2 py-1 rounded-full bg-surface-elevated text-primary border border-primary text-xs font-medium">{t.name}</span>)}</div>;
                  ...
                  {card ? (
                    <div className="space-y-3">
                      <div className="relative">
                        ...
                        <button
                          ...
                          onClick={(e) => { e.stopPropagation(); e.preventDefault(); if (slots[index]) toggleCard(slots[index]); }}
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="space-y-2">
                        <Popover open={searchResults[index].length > 0}>
